...
app = FastAPI(
    ...
    description="Sub-100ms full-duplex WebSockets voice intelligence engine with VAD, emotion intent classification, barge-in handling & SLA latency benchmarks.",
    version="3.2.0"
)
...
async def health_check():
    return {"status": "healthy", "engine": "multimodal-voice-ai-engine", "latency_target": "<100ms", "version": "3.2.0"}


@app.get("/api/v1/voice/benchmark", response_model=LatencyBenchmark)
async def get_latency_benchmark():
    """Returns P50, P90, P99 turn-taking latency benchmark metrics."""
